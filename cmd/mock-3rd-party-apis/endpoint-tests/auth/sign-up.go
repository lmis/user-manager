...
func TestSignUp(config *config.Config, emails map[string][]email_api.EmailTO) error {
	...
	if err = test_util.AssertResponseEq(204, nil, resp); err != nil {
		return util.Wrap("signup response mismatch", err)
	}

	// Login
	resp, err = test_util.MakeApiRequest("POST", config, "auth/login", auth_endpoint.LoginTO{
		Email:    email,
		Password: password,
	}, nil)
	if err != nil {
	...
	if err = test_util.AssertResponseEq(200, auth_endpoint.LoginResponseTO{Status: auth_endpoint.LoggedIn}, resp); err != nil {
	...
	// Check role
	resp, err = test_util.MakeApiRequest("GET", config, "role", auth_endpoint.LoginTO{
		Email:    email,
		Password: password,
	}, sessionCookie)
	if err != nil {
	...
	if err = test_util.AssertResponseEq(200, api_endpoint.AuthRoleTO{Roles: []models.UserRole{"USER"}}, resp); err != nil {
	...
	// Confirm with token
	resp, err = test_util.MakeApiRequest("POST", config, "user/confirm-email", user_endpoint.EmailConfirmationTO{
		Token: token,
	}, sessionCookie)
	if err != nil {
		return util.Wrap("error making confirm email call", err)
	}
	if err = test_util.AssertResponseEq(200, user_endpoint.EmailConfirmationResponseTO{Status: "newly-confirmed"}, resp); err != nil {
	...
	if err = test_util.AssertResponseEq(200, user_endpoint.EmailConfirmationResponseTO{Status: "already-confirmed"}, resp); err != nil {
	...
	if err = test_util.AssertResponseEq(200, api_endpoint.AuthRoleTO{Roles: []models.UserRole{"USER"}, EmailVerified: true}, resp); err != nil {
	...
	if err = test_util.AssertResponseEq(200, api_endpoint.AuthRoleTO{Roles: nil}, resp); err != nil {
